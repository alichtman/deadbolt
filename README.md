...
`deadbolt` is built on Electron and uses `crypto.js` from the `node.js` standard library as well as the [`node-argon2` library](https://www.npmjs.com/package/argon2). `AES-256-GCM` is used as an encryption protocol, and `argon2id` is used as a password hashing function. The integrity of all encrypted data is verified with the authentication tag provided by AES-GCM mode.
