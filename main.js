...
function decryptFile(filePath, decryptionKey, event) {
	...
	readMetadata.on("close", () => {
		...
		const decryptedFileWriteStream =
...
app.on("ready", () => {
	ipcMain.handle("encryptFileRequest", () => encryptFile(filePath, password));
	ipcMain.handle("decryptFileRequest", () => decryptFile(filePath, password));
	createWindow();
});
app.on("activate", () => {
	if (mainWindow === null) {
